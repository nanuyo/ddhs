...
def change_file_permissions(file_path):
    """파일 권한을 읽기 가능으로 변경합니다."""
    try:
        os.chmod(file_path, stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IROTH)
        logger.info(f"Permissions for {file_path} changed successfully.")
    except Exception as e:
        logger.error(f"Failed to change permissions for {file_path}: {e}")

def load_json_file(file_path):
    """JSON 파일을 읽고 내용을 반환합니다."""
    change_file_permissions(file_path)  # 권한 변경 시도
    try:
...
MAPPING_TABLE_FILE = '/usr/bin/ims/uart/mapping_table.json'  # 수신용 mapping table
SEND_MAPPING_TABLE_FILE = '/usr/bin/ims/uart/send_mapping_table.json'  # 송신용 mapping table
SERVER_JSON_DIR = '/usr/bin/ims/uart/from_server/'  # 서버에서 받은 JSON 파일이 저장되는 디렉토리
BASE_DIRECTORY = '/usr/bin/ims/uart/to_server'  # 수신 데이터 저장 기본 디렉토리, 서버로 전송할거임
...
def process_received_data(data, mapping_table):
    ...
    for id_addr, dec_value in extracted_data:
        # RUN SOFTAP when ID_ADDR is 0086 and dec_value is 1
        if id_addr == "0086" and dec_value == 1:

    ...
    if ser.is_open:
        try:
            ...
            if received_data:
                ...
                hex_data = ' '.join(f'{byte:02X}' for byte in received_data)  # 16진수로 변환
                print(f'Received (hex): {hex_data}')  # 16진수 출력 추가
                try:
                    decoded_data = received_data.decode(errors='ignore').strip()

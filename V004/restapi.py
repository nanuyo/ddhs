...
class SimpleHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            if self.path.startswith('/status'):
                ...
                if userid == "test_user":  # 여기를 실제 사용자 ID 검사 로직으로 대체해야 함
    ...
    def do_POST(self):
        try:
            ...
            # ssid, password, userid가 모두 제공되었는지 확인합니다.
            if not ssid or not password or not userid:
                self.respond(499, 'WiFi Config Error')
                return

            try:
                # WiFi 설정을 저장하고 시스템을 재부팅합니다.
                save_wifi_config(ssid, password)
                self.respond(200, 'WiFi configuration received. The system will reboot.', {'deviceid': DEVICE_ID})
                reboot_system()
            except Exception as e:
